    ...
    function Store(preLoadState) {
        ...
        this._validateAll = function () {
            ...
            Object.keys(current).forEach(function (item) {
                descriptor[item] = current[item].validate.map(function (val) {
                    return __assign(__assign({}, val), { pattern: val.pattern ? new RegExp(val.pattern[0]) : '' });
                });
        ...
        this._validateOne = function (field) {
            ...
            descriptor[field] =
                current.validate &&
                    current.validate.map(function (v) {
                        return __assign(__assign({}, v), { pattern: v.pattern ? new RegExp(v.pattern[0]) : '' });
                    });
