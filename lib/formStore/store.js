...
var Store = /** @class */ (function () {
    function Store(preLoadState) {
        ...
        this.setInitialValues = function (payload) {
            _this.dispatch(__assign({ type: 'set' }, payload));
        };
        ...
        this._validateOne = function (field) {
            ...
            validator
                ...
                for (var i = 0; i < _this.subscribeList.length; i++) {
                    var name = Object.keys(_this.subscribeList[i])[0];
                    if (field === name) {
                        _this.subscribeList[i][name]();
                    }
                }
            })
                .catch(function (_a) {
                var errors = _a.errors, fields = _a.fields;
                _this.dispatch(__assign({ type: 'upDateMessage' }, fields));
                for (var i = 0; i < _this.subscribeList.length; i++) {
                    var name = Object.keys(_this.subscribeList[i])[0];
                    if (field === name) {
                        _this.subscribeList[i][name]();
            ...
            switch (action.type) {
                ...
                case 'set':
                    ...
                    for (var i = 0; i < _this.collectionRulesStoreList.length; i++) {
                        var name = Object.keys(_this.collectionRulesStoreList[i])[0];
                        if (Object.keys(action)[1] === name) {
                            _this.collectionRulesStoreList[i][name]();
                        }
                    }
                    _this._validateAll();
                    break;
                // 更新error message
                case 'upDateMessage':
                    _this._currentState = _this._reducer(_this._currentState, action_1.upDateMessage_action(action));
                    break;
            }
        };
